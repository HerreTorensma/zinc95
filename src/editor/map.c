...
static void draw_grid(framebuffer_t *fb) {
	int line_x = SCREEN_WIDTH - cam_pos.x % SCREEN_WIDTH;
	int line_y = SCREEN_HEIGHT - cam_pos.y % SCREEN_HEIGHT;
...
void map_editor_draw(computer_t *computer) {
	...
	// yeah
	int map_x = cam_pos.x / SPRITE_WIDTH;
	int map_y = cam_pos.y / SPRITE_HEIGHT;
	// 81 so the screen is always filled

	for (int i = 0; i < MAP_LAYERS_AMOUNT; i++) {
		gfx_draw_map(computer->ram, i, POINT(-cam_pos.x, -cam_pos.y), RECT(map_x, map_y, 81, 60));
	}
